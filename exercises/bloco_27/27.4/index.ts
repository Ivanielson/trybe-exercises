...
class Plants {
  ...
  async getPlants() {
    const plantsRaw = await fs.readFile(FILE_PATH, { encoding: 'utf8' });
  ...
  async removePlantById(id: string) {
    ...
    await fs.writeFile(FILE_PATH, JSON.stringify(newPlants));
  ...
  async editPlant(plantId: string, newPlant: IPlant) {
    ...
    await fs.writeFile(FILE_PATH, JSON.stringify(updatedPlants));
    return newPlant;
  }
